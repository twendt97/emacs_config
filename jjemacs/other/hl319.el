;;            /---"Exp" means Experimental...  I don't have time to work on this now
;;	     V	 			       and so I don't use it much...

;; hl319.el (Exp 1.33) -- customizable highlighting for Emacs19.
;; Copyright (c) 1993 Free Software Foundation, Inc.
;; Copyright (C) 1994 Jonathan Stigelman <Stig@hackvan.com>
;;
...
;;
;; fix (add-patterns) so that it works with the new pattern compilation
;; mechanisms.
;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;; hl319.el,v 1.33 1995/03/02 07:37:13 stig Exp
;;
;; LCD Archive Entry:
;; hl319|Jonathan Stigelman|Stig@hackvan.com|
;; Comprehensive (and comparatively fast) regex-based highlighting for Emacs 19|
;; 1995/03/02 07:37:13|Exp 1.33|~/packages/hl319.el.Z|
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;; GENERAL OVERVIEW
...
;;
;; hl319.el,v
;; Revision 1.33  1995/03/02 07:37:13  stig
;; foo
;;
;; Revision 1.32  1995/01/27 11:01:09  stig
;; XEmacs compatibility stuff (not used much) and some pattern changes
;;
...
(defvar hilit-hook-list '(yank yank-pop find-file info mail gnus dired change)
...
(defconst hilit-default-face-table
  '(
    ...
    (marked	  purple	     Plum1	       nil)
    (ignored	  ForestGreen	     moccasin	       nil)
    (deleted	  red-bold-italic    orange-italic     bold-italic)

    (crossref	  DarkGoldenrod	     Goldenrod	       underline)
    (directory	  blue-bold	     cyan-bold	       bold)
    (link	  firebrick-italic   green-italic      italic)
    ...
    (rule	  blue-bold-underline cyan-bold-underline default-bold-underline)
    ...
    (msg-subject    blue-bold       yellow-bold	       bold)
    (msg-from	    purple-bold	    green-bold	       bold)
    (msg-header	    firebrick-bold  cyan-bold	       italic)
    (msg-separator  black/tan-bold  black/lightblue-bold nil)
    ...
    (jargon-entry    blue-bold	       cyan-bold       bold)
    (jargon-xref     purple-bold       Plum1-bold      italic)
    (jargon-keyword  firebrick-underline yellow-underline underline)
    )
	...
	(error (message "couldn't allocate color for '%s'"
	  ...
	  (progn
		...
		(progn
		  ...
		  (set-face-font face (face-font face))))
...
(cond ((string-match "XEmacs\\|Lucid" emacs-version)
       ;; FIXME - will the byte compiler optimize these if they're not
       ;; top-level?  gotta check on that...
...
The optional 5th arg, PROP is a property to set instead of 'hilit."
	 (let ((ext (make-extent start end)))
	   (set-extent-face ext face-name)
	   (set-extent-property ext (or prop 'hilit) t)
	   (and prio (set-extent-priority ext prio))
	   ))
	 ...
	 (map-extents (function (lambda (ext foo)
				  (and (extent-property ext 'hilit)
				       (delete-extent ext))
				  nil))
		      nil start end)
	 (or quietly hilit-quietly (message "Done unhighlighting")))
      ...
      (t 
       ;; FSFmacs lossage...  (text-properties and overlays sucks)
       (defsubst hilit-region-set-face (start end face-name &optional prio prop)
	 "Highlight region from START to END using FACE and, optionally, PRIO.
The optional 5th arg, PROP is a property to set instead of 'hilit."
...
(cond ((string-match "XEmacs" emacs-version)
	 ...
	 (save-restriction
	   ...
	   (map-extents (function (lambda (e x)
				    (setq x (extent-start-position e))
				    (and (< x start) (setq start x))
				    nil))
			nil start start)
	   (map-extents (function (lambda (e x)
				    (setq x (extent-start-position e))
				    (and (> x end) (setq end x))
				    nil))
			nil end end)
	   (hilit-unhighlight-region start end quietly)
	   (hilit-highlight-region   start end nil quietly)))
...
(defun hilit-submit-feedback ()
  "Submit feedback on hl319 to the author: Stig@hackvan.com"
  (interactive)
  (require 'reporter)
...
hilit-default-face-table"
  (setq hilit-face-translation-table
	(let ((index (or (and (or (x-display-color-p)
				  (ns-display-color-p))
...
(defun hilit-initialize ()
  ...
  (if (not window-system)
    ...
    ;;     
    (cond ((string-match "XEmacs" emacs-version)
	   (global-set-key [(control L)] 'hilit-repaint-command)
	   (global-set-key [(control =)] 'hilit-repaint-command))
	  (t
    ...
    (hilit-hook-install mail
      (mapcar (function (lambda (hook)
		...
		mh-show-mode-hook	       mh-letter-mode-hook))
      (add-hook 'vm-summary-mode-hook 'hilit-install-line-hooks)
      (add-hook 'vm-summary-pointer-update-hook 'hilit-install-line-hooks)
    ...
    (hilit-hook-install change
      (add-hook 'dired-mode-hook 'hilit-install-line-hooks)
      (add-hook 'find-file-hooks 'hilit-install-line-hooks)
      (add-hook 'emacs-lisp-mode-hook 'hilit-install-line-hooks)
      (add-hook 'lisp-mode-hook 'hilit-install-line-hooks) ; FIXME - ???
      )
...
(let ((c-comments     '(("/\\* \\(FIXME\\|fixme\\)" "\\*/" urgent) ; this winds up being 2 overlays
			("/\\*" "\\*/" comment)))
      ;; FIXME - also highlight character constants as strings.
...
(let* ((header-patterns (` ((:buildme:
  ...
  (hilit-set-mode-patterns '(vm-mode vm-virtual-mode text-mode mail-mode
			     rmail-mode news-reply-mode mh-show-mode mh-letter-mode)
			   message-patterns
...
(hilit-set-mode-patterns
 ...
 '(
   (:buildme:
    ;; FIXME - These are all "[^\n]" instead of "." to work around an fsf bug
    (summary-current "^. +[0-9]+:\\+\\[[^\n]*$") 
    (summary-seen    "^D +[0-9]+: \\[[^\n]*$")
...
(hilit-set-mode-patterns
 ...
 '((:buildme:
    ...
    (:and: "^[ \t]*" (jargon-keyword "[*][A-Za-z][^*]+[*]")) 
    ;; keywords for the emacs & lisp manuals
    (jargon-keyword (:and: "- " (:or: "Variable" "Function" "Macro" "Command"
...
(hilit-set-mode-patterns
 ...
 '(("(\\*" "\\*)" comment)
   ...
   ("\\<\\(RECORD\\|ARRAY\\|OF\\|POINTER\\|TO\\|BEGIN\\|END\\|FOR\\|IF\\|THEN\\|ELSE\\|ELSIF\\|CASE\\|WHILE\\|DO\\|MODULE\\|FROM\\|RETURN\\|IMPORT\\|EXPORT\\|VAR\\|LOOP\\|UNTIL\\|\\DEFINITION\\|IMPLEMENTATION\\|AND\\|OR\\|NOT\\|CONST\\|TYPE\\|QUALIFIED\\|REPEAT\\|EXIT\\|WITH\\)\\>" nil keyword)
   )
  nil 'case-insensitive)
...
(hilit-set-mode-patterns
 ...
 '(
   ...
   ("\\\\documentstyle\\(\\[.*\\]\\)?{" "}" decl)
   ("\\\\\\(begin\\|end\\|includeonly\\){" "}" decl)
   ("\\\\\\(raggedright\\|makeindex\\|makeglossary\\|maketitle\\|nofiles\\)\\b" nil
    decl)
   ...
   ("\\\\\\(appendix\\|tableofcontents\\|listoffigures\\|listoftables\\)\\b"
    nil decl)
   ("\\\\\\(bf\\|em\\|it\\|rm\\|sf\\|sl\\|sc\\|tt\\|sc\\)\\b" nil decl)
...
;;;   "OVERLOADED BY HILIT19 (for testing purposes): Toggle highlighting of buffer"
;;; ;; "
...
;; From: marce@dit.upm.es (Marcelino Veiga Tuimil)
;; To: xemacs@cs.uiuc.edu
;; Subject: Re: hilit19 question
;; Date: Wed, 2 Nov 1994 11:49:06 GMT
;; 
;; In article <WING.94Oct27025233@mteverest.spg.amdahl.com> wing@mteverest.spg.amdahl.com (Ben Wing) writes:
;; 
;;   | >>>>> "Tim" == Tim Gray <consp07@bingsuns.cc.binghamton.edu> writes:
;;   | 
;;   |     Tim> How can I customize my colors when I am using hilit19?  Right
;;   |     Tim> now it gives me white, blue, red, and a dark gray.  I want
;;   |     Tim> some colors that will stand out a little better against a
;;   |     Tim> black background.  I am programming in C if that matters.
;;   |     Tim> For those of you who don't know whay hilit19 is, it is a
;;   |     Tim> library that allows you to do text and keyword highlighting
;;   |     Tim> in a few different programming languages( makes your comments
;;   |     Tim> red, your keywords blue, that sort of thing).
;; 
;; I have found a nice solution! If your add following code to your ~/.emacs:
;; 
;; (if (equal window-system 'x)
;;     (progn
;;       (defun x-hilit-translate (face)
;;         (let ((component "hilit19")
;;               (attribute (symbol-name face)))
;;           (let ((color (x-get-resource attribute
;;                                        (capitalize component)
;;                                        component
;;                                        (capitalize attribute))))
;;             (if color
;;                 (eval (list 'hilit-translate face '(intern color)))))))
;; 
;;       (defun x-do-hilit-translate ()
;;         (mapcar 'x-hilit-translate
;;                 (mapcar 'car hilit-face-translation-table)))
;; 
;;       (if (featurep 'hilit19)
;;           (x-do-hilit-translate)
;;         (eval-after-load "hilit19" '(x-do-hilit-translate))))
;;   )
;; 
;; You can change hilit colour using X resources like:
;; 
;; Emacs.hilit19.comment: bold
;; Emacs.hilit19.defun: bold
;; 
;; To see current color settings, try:
;; 
;; C-h v hilit-face-translation-table RET
;; 
;; And to see what keywords apply to a major mode:
;; 
;; C-h v hilit-patterns-alist RET
;; 
